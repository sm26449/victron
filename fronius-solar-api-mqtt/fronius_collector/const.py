"""Constants for Fronius Collector."""
...
def get_status_description(status_code: int) -> str:
    ...
    if status_code in STATUS_CODES:
        return STATUS_CODES[status_code]["description"]
...
def get_status_name(status_code: int) -> str:
    """Get status name."""
    if status_code in STATUS_CODES:
        return STATUS_CODES[status_code]["name"]
    return f"UNKNOWN_{status_code}"


def is_status_alarm(status_code: int) -> bool:
    """Check if status code indicates an alarm condition."""
    if status_code in STATUS_CODES:
        return STATUS_CODES[status_code]["alarm"]
    return False
